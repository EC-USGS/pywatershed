...
class ColorBrewer:
    def __init__(self):
        ...
        for cat in ColorBrewer.category_list():
            setattr(self, cat, self.palettes_by_cat[cat])
        return

    @staticmethod
    def category_list():
        return list(palette_names_by_cat.keys())

    @staticmethod
    def palette_dict():
        return palette_names_by_cat

    @staticmethod
    def get_palette(name):
        for cat in ColorBrewer.category_list():
            if name in palettes_by_cat[cat].keys():

                def pal(n_levels: int = None, rgb: bool = False):
                    pal_name = name
                    pal = palettes_by_cat[cat][name]
                    if not n_levels:
                        print(
                            f"For palette {pal_name}, supply number of "
                            f"levels in {list(pal.keys())}"
                        )
                        return None
                    if rgb:
                        return pal[n_levels]
                    else:
                        return ColorBrewer._to_hex(pal[n_levels])

                return pal

    def set_format(self, format: str):
        if format not in self._possible_formats:
            raise ValueError(
                f"{format} not in possible formats: {self._possible_formats}"
            )
        self.format = format
        return
    ...
    @staticmethod
    def _to_hex(rgb_palette):
        return ["#%02x%02x%02x" % rgb_tuple for rgb_tuple in rgb_palette]
...
palette_names_by_cat = {
    "sequential": sequential.keys(),
    "diverging": diverging.keys(),
    "qualitative": qualitative.keys(),
}
