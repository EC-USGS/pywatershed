...
def get_diff_tol_df(var_df: pd.DataFrame, var_name: str, tol: np.float64) -> pd.DataFrame:
    ...
    diff_df = var_df.copy()
    diff_df['diff'] = np.where(diff_mask & rel_diff_mask, var_df['diff'], np.nan)
    diff_df['rel_diff'] = np.where(diff_mask & rel_diff_mask, var_df['rel_diff'], np.nan)
...
def get_proc_var_dict(model: pynhm.Model, ds: xr.Dataset) -> dict:
    ...
    avail_vars = list(set(['_'.join(vv.split('_')[0:-1]) for vv in avail_vars if vv not in drop_vars]))
    proc_var_dict = {}
    for proc_key, proc_val in model.processes.items():
        proc_var_dict[proc_key] = [var for var in proc_val.get_variables() if var in avail_vars]
...
def get_stat_location_id(stat_df, stat_name, tap_value, space_coord):
    ...
    return stat_df[abs(stat_df['diff']) == abs(stat_df['diff']).min()][space_coord].values[0]
...
def err_panel(
    ...
    """A panel dashboard for exploring error distributions and drilling down to individual locations
    for details on their timeseries and errors"""
    
    var_list = [var for proc_key, proc_val in model.processes.items() for var in proc_val.get_variables()]
    ...
    def get_stat_df(stat_name, var_name):
        ...
        var_ds = data_ds[[f"{var_name}_pynhm", f"{var_name}_prms"]]
        stat_df = metrics.stat_dict[stat_name](
            mod=data_ds[f"{var_name}_pynhm"], 
            obs=data_ds[f"{var_name}_prms"], 
            dim=time_coord).to_dataframe(name=stat_name).reset_index()
