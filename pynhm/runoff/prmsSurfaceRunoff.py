...
class prmsSurfaceRunoff(StorageUnit):
    def __init__(
        ...
        self.hru_percent_imperv = parameters["hru_percent_imperv"]
        if self.hru_percent_imperv.sum() > 0.:
            self.impervious = True
            self.imperv_stor_max = parameters["imperv_stor_max"]
            self.imperv_stor_new = parameters["imperv_stor_start"]
            self.imperv_stor_old = parameters["imperv_stor_start"]
        else:
            self.impervious = False
            self.imperv_stor_max = None
            self.imperv_stor_new = None
            self.imperv_stor_old = None

        self.carea_min = parameters["carea_min"]
        self.carea_max = parameters["carea_max"]
        self.soil_rech_max_frac = parameters["soil_rechr_max_frac"]
        self.smidx_coef = parameters["smidx_coef"]
        self.smidx_exp = parameters["smidx_exp"]
        super().__init__("sro", id, parameters["hru_area"], forcing, verbose)
    ...
    def advance(self, itime_step):
        ...
        if self.impervious:
            self.imperv_stor_old = self.imperv_stor_new.copy()
        return
...
class prmsSurfaceRunoff_deprecated(StorageUnit):
    def __init__(
        self,
        id,
        area,
